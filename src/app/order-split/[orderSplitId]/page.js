...
import { completeOrder, fetchOrderDetail, getMyInvoice } from "@/store/slices/orderSlice"
...
function OrderSplitDetail({ params }) {
    ...
    const handleChangeStatus = async () => {
        try {
            const response = await dispatch(completeOrder(orderSplitId)).unwrap()
    ...
    return (
        <div>
            ...
            <div className="flex items-center border-[#F0F3F7] px-4 py-3 lg:hidden">
                ...
                <div className="ml-2 font-semibold">{t("orderDetail.detailSplitPayment")}</div>
            </div>
            <div className="mx-auto min-h-screen max-w-7xl lg:flex">
                ...
                <div className="w-5/5 py-4 lg:w-4/5 lg:p-7 lg:px-4">
                    <div className="hidden items-center lg:flex">
                        ...
                        <div className="pb-1 text-2xl font-bold">{t("orderDetail.detailSplitPayment")}</div>
                    </div>
                    <div className="flex flex-col lg:my-7 lg:flex-row lg:gap-4">
                                ...
                                {order &&
                                    ...
                                    order.items.map((item, index) => (
                                        <div key={index} className="flex items-center justify-between py-2">
                                            ...
                                            {order?.order_status?.value === "completed" && !order?.has_reviewed && user?.data?.id === order?.user?.id && (
                                                <Link href={`/review-create?orderId=${orderSplitId}&productId=${item?.product?.id}`}>
                            ...
                            <div className="rounded-xl bg-white px-5 py-4 text-sm lg:shadow">
                                ...
                                <div className="border-b py-2 text-sm">
                                    ...
                                    <div className="flex items-center p-1">
                                        ...
                                        <div className="w-1/2">{order > 0 ? (order?.invoices[0]?.payment_method?.name === null ? "-" : order?.invoices[0]?.payment_method?.name) : "-"}</div>
                                    </div>
                                    <div className="flex items-center p-1">
                                    ...
                                    <div className="my-2 mb-4 flex items-center justify-between p-1 text-sm">
                                        <div>
                                            <div className="text-[#6D7588] lg:w-1/2">Split Link</div>
                                            <div className="text-[#007185] lg:w-1/2">{myInvoice && myInvoice?.payment_url}</div>
                                        </div>
                                        <div className="ml-5 flex cursor-pointer items-center text-right text-sm" onClick={() => copyToClipboard(myInvoice?.payment_url)}>
                                            <div className="mr-1 text-sm font-bold text-[#007185]">{t("orderDetail.copy")}</div>
                                ...
                                {order?.payment_status?.value === "PENDING" && (
                                    <div onClick={() => handleToPayment()} className="my-2 cursor-pointer items-center justify-center rounded-lg bg-secondary px-6 py-3 text-center text-sm font-bold hover:bg-[#e8bc00]">
                                        {t("orderDetail.payNow")}
                                    </div>
                                )}

                                {order?.order_status?.value === "delivered" && user?.data?.id === order?.user?.id && (
                        ...
                        <div className="h-full lg:w-1/2">
                            ...
                            <div className="my-2 rounded-xl bg-white px-5 py-4 shadow">
                                <div className="mb-2 font-extrabold">{t("orderDetail.statusPayment")}</div>
                                {order &&
                                    order?.invoices &&
                            ...
                            <div className="rounded-xl bg-white px-5 py-4 shadow">
                                <div className="flex items-center justify-between">
                                    <div className="text-sm">{t("orderDetail.remainingPayment")}</div>
                                    <div className="font-extrabold">
                                        {/* {order && order.total_price?.formatted} */}
                                        {order && order?.remaining_amount?.formatted}
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            {/* <Footer /> */}
            <PopupModal isOpen={notification} closeModal={() => setNotfication(false)} type={"notification"} title={"Pemberitahuan"} message={`Berhasil konfirmasi pesanan.`} />
