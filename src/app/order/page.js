...
import { ArrowLeftIcon, ChevronDownIcon, ChevronUpIcon, XMarkIcon } from "@heroicons/react/24/solid"
    ...
    return (
        <div>
            ...
            <div className="flex items-center border-[#F0F3F7] px-4 py-3 lg:hidden">
                ...
                <div className="ml-2 font-semibold">{t("listOrder.orderStatus")}</div>
            </div>
            <div className="mx-auto min-h-screen max-w-7xl lg:flex">
                ...
                <div className="w-5/5 py-4 lg:w-4/5 lg:p-7 lg:px-4">
                    <div className="hidden pb-1 text-2xl font-bold lg:block">{t("listOrder.orderStatus")}</div>
                    <div className="mt-4 flex flex-col items-center px-4 lg:mt-10 lg:flex-row lg:px-0">
                        <div className="item-center mr-2 w-full lg:mb-0 lg:flex lg:w-8/12">
                            <input className="mr-4 w-full rounded-lg border border-[#BFBFBF] py-2 pl-14 text-black bg-search focus:border-secondary focus:ring-0" placeholder="Cari pesananmu" onChange={handleSearchChange} />
                        </div>
                        <div className="item-center hidden w-full lg:flex lg:w-4/12">
                            <input className="w-full rounded-lg border border-[#BFBFBF] py-2 text-black focus:border-secondary focus:ring-0" placeholder="Pilih Tanggal Transaksi" type="date" onChange={handleDateChange} />
                        </div>
                    </div>
                    <div className="mt-8 hidden items-center lg:flex">
                        ...
                        <div
                            onClick={() => handleStatusChange("waiting_confirmation")}
                    ...
                    {/* Mobile View */}
                    <div className="mt-4 flex items-center justify-between overflow-x-auto whitespace-nowrap px-4 lg:hidden">
                        <div className="rounded-full bg-[#F5F5F5] p-2 text-[#6D7588]" onClick={handleClearFilter}>
                            <XMarkIcon className="h-6 w-6" />
                        </div>

                        <div onClick={() => setIsOpenStatusMobile(!isOpenStatusMobile)} className="mx-2 flex cursor-pointer items-center rounded-full border border-[#007185] bg-[#F4FDFF] px-4 py-2 text-sm font-semibold text-[#007185]">
                            {descStatus || "Pilih Status"}
                            {isOpenStatusMobile ? <ChevronUpIcon className="ml-2 h-6 w-6" /> : <ChevronDownIcon className="ml-2 h-6 w-6" />}
                        </div>

                        <input className="w-full rounded-full border border-[#BFBFBF] py-2 text-black focus:border-secondary focus:ring-0" placeholder="Pilih Tanggal Transaksi" type="date" onChange={handleDateChange} />
                    </div>
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                            <div className="w-full max-w-md rounded-lg bg-white p-4">
                                <div className="mb-4 flex items-center justify-between">
                                    <div className="text-lg font-semibold">{t("listOrder.selectStatus")}</div>
                                    <XMarkIcon className="h-6 w-6 cursor-pointer" onClick={() => setIsOpenStatusMobile(false)} />
                                </div>
                                <div className="space-y-2">
                                    <div
                                        onClick={() => handleStatusChange(null, "Semua")}
                                    ...
                                    <div
                                        onClick={() => handleStatusChange("waiting_confirmation", "Menunggu Konfirmasi")}
                                    ...
                                    <div
                                        onClick={() => handleStatusChange("processing", "Diproses")}
                                    ...
                                    <div
                                        onClick={() => handleStatusChange("shipped", "Dikirim")}
                                    ...
                                    <div
                                        onClick={() => handleStatusChange("delivered", "Tiba di Tujuan")}
                                    ...
                                    <div
                                        onClick={() => handleStatusChange("canceled", "Dibatalkan")}
                    ...
                    {orders.map((order, index) => (
                        <div key={index} className="hidden items-center border-b border-[#F0F3F7] bg-white px-5 py-4 lg:flex">
                            <div className="w-1/2 border-r">
                                <div className="flex items-center">
                                    <div>
                                        <Image src={order.items_count > 0 && order.items[0].product.images[0]} width={100} height={100} alt="cart-product" priority={false} />
                                    </div>
                                    <div className="ml-5 text-sm leading-6">
                                        <div className="text-md pb-1">{order.items_count > 0 && Cookies.get("locale") === "en" ? (order.items[0].product?.name_trans?.en ? order.items[0].product.name_trans.en : order.items[0].product?.name_trans?.id) : order.items[0].product?.name_trans?.id}</div>
                                        <div className="text-md font-bold">{order.total_price.formatted}</div>
                                    </div>
                                </div>
                            </div>
                            <div className="w-1/2">
                                <div className="flex items-center justify-between">
                                    <div className="ml-5 text-sm leading-6">
                                        <div className="text-md pb-1">{t("listOrder.orderStatus")}:</div>
                                        <div className={`w-fit rounded-lg bg-[${order.order_status.color}] px-2 py-1 text-xs font-semibold text-[#007185]`}>{order.order_status.label}</div>
                                    </div>
                                    <Link href={`/order/${order.id}`}>
                                        <div className="cursor-pointer items-center justify-center rounded-lg bg-secondary px-6 py-2 text-center text-sm font-bold hover:bg-[#e8bc00]">{t("listOrder.trackOrder")}</div>
                                    </Link>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Start : View Mobile */}
                    {orders.map((order, index) => (
                        <div key={index} className="m-4 flex flex-col items-center rounded-xl bg-white px-5 py-4 shadow lg:hidden" onClick={() => router.push(`/order/${order.id}`)}>
                            <div className="flex w-full items-center">
                                <div className="w-1/3">
                                    <Image src={order.items_count > 0 && order.items[0].product.images[0]} width={100} height={100} alt="cart-product" priority={false} />
                                </div>
                                <div className="ml-5 w-2/3 text-sm leading-6">
                                    <div className="text-md pb-1">{order.items_count > 0 && order.items[0].product.name}</div>
                                    <div className="text-md font-bold">{order.total_price.formatted}</div>
                                    <div className={`mt-2 w-fit rounded-xl bg-[${order.order_status.color}] px-2 py-1 text-xs font-semibold text-[#007185]`}>{order.order_status.label}</div>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {orders && orders.length > 15 && <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={handlePageChange} />}
            <Footer />
